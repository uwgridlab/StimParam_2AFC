%% StimParam_2AFC_run_CheckParams
% Written: 5/12/18
% J Cronin
...
% This code will check the parameters that you're going to use in the
% StimParam_2AFC_run tests 
clc; clearvars; close all


%% Set experimental parameters
numSets = 2; % number of sets of the unique stim parameter trials that you want to do
...
paramsReadIn = xlsread('StimParameters_for2AFC.xlsx', 2, 'B3:I5'); % read in from the second sheet which just includes the test parameters
labels = [1 2 3]'; % Need to change this if I update the number of trials
...
for i=1:numTrials
    ...
    if strcmp(resp,'left') % Left response is A (Intensity question)
        response(i) = 1;
        displayTrial = ['Trial #', num2str(i), ': A'];
    elseif strcmp(resp,'right') % Right response is B (Intensity question)
        response(i) = 0;
