...
responseIntensity = zeros(numTrials, 1);
resultsIntensity = zeros(numTrials, 1);
responseSameDiff = zeros(numTrials, 1);
...
for i=1:numTrials
    ...
    % values)
    trainA_chargeSet(i) = paramsTest(i,1)/1000000*paramsTest(i,2)*paramsTest(i,5); % in Amp*us
    trainB_chargeSet(i) = paramsTest(i,6)/1000000*paramsTest(i,7)*paramsTest(i,10);
    ...
    trainA_chargeDelivered(i) = paramsTest_actual(i,1)/1000*paramsTest_actual(i,2)*paramsTest_actual(i,5);
    trainB_chargeDelivered(i) = paramsTest_actual(i,6)/1000*paramsTest_actual(i,7)*paramsTest_actual(i,10);
    ...
    % Get answer from subject on the intensity question
    ...
    if strcmp(resp,'left')
        responseIntensity(i) = 1;
    elseif strcmp(resp,'right')
        responseIntensity(i) = 0;
    else
    ...
    pause(0.5)
    DA.SetTargetVal('RZ5D.leftIndicator', 0);
    DA.SetTargetVal('RZ5D.rightIndicator', 0);
    
    % Get answer from subject on the Same/Diff question
    ...
    while exitKeyboardLoop == 0
        ...
        elseif keyCode(leftKey) % subject indicated left-side answer
            keyboardResp = 1;
            exitKeyboardLoop = 1;
            vbl = displaySameDiffQuestion(window, white, screenXpixels, screenYpixels, 'left'); % 'left' will color code the left-side answer
            
        elseif keyCode(rightKey) % subject indicated right-side answer
            keyboardResp = 0;
            exitKeyboardLoop = 1;
            vbl = displaySameDiffQuestion(window, white, screenXpixels, screenYpixels, 'right'); % 'right' will color code the right-side answer
    ...
    % correct or incorrect
    if strcmp(resp,'left')
        responseSameDiff(i) = 1;
    elseif strcmp(resp,'right')
        responseSameDiff(i) = 0;
    else
        warning('Something went wrong in going thru yes/no steps')
        return
    end
    ...
    % For intensity question/response, consider if the reponse is correct
    % or not
    if responseIntensity(i)==1 && trainA_chargeDelivered(i)>=trainB_chargeDelivered(i)
        resultsIntensity(i)=1;
    elseif responseIntensity(i)==0 && trainB_chargeDelivered(i)>trainA_chargeDelivered(i)
        resultsIntensity(i)=1;
    else
        resultsIntensity(i)=0;
    end
    
    % Display response in command window
    if responseIntensity(i)==1 && responseSameDiff(i)==1
        displayTrial = sprintf('Trial #%.0f: A, Same', i);
    elseif responseIntensity(i)==1 && responseSameDiff(i)==0
        displayTrial = sprintf('Trial #%.0f: A, Different', i);
    elseif responseIntensity(i)==0 && responseSameDiff(i)==1
        displayTrial = sprintf('Trial #%.0f: B, Same', i);
    elseif responseIntensity(i)==0 && responseSameDiff(i)==0
        displayTrial = sprintf('Trial #%.0f: B, Different', i);
    else
        displayTrial = sprintf('Trial #%.0f: Did not process answer correctly', i);
    end
    
    if resultsIntensity(i)==1 % Correct answer
        disp([displayTrial, sprintf(', Charge = %.0f vs. %.0f A*us, Correct', trainA_chargeDelivered(i), trainB_chargeDelivered(i))])
    else % Incorrect answer 
        disp([displayTrial, sprintf(', Charge = %.0f vs. %.0f A*us, Incorrect', trainA_chargeDelivered(i), trainB_chargeDelivered(i))])
    end
       
    % Now that subject has indicated with a keyboard press,
    ...
    pause(0.5)
    DA.SetTargetVal('RZ5D.Indicator', 0); % set the indicator back to 0
...
for i=1:3 % loop through the three different parameters that were changed
    ...
    results_Labeled{i} = resultsIntensity(labelsTest==i);
    responseSameDiff_Labeled{i} = responseSameDiff(labelsTest==i);
