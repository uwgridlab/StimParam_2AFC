...
singleJND = 1; 
% if this is set to 1 then it will only read in the first 5 rows of stim
% values from Excel. This should be set to 1 if you're running a pilot test
% or if you only want to test a single static variable

%NOTE - I don't think the repelem for labels below works properly when
%singleJND is set to 1 so it requires more post processing - you won't lose
%data though

numSets = 5; % check with this first
...
ISI = 2000; % ms (usually run this at 2000 ms); this is the time between the end of the first stim train and the start of the second stim train
...
if singleJND==0
    paramsReadIn = xlsread('StimParameters_for2AFC.xlsx', 3, 'B3:I12');
    labels = repelem([1 2]',5); % Need to change this if I update the number of trials
    % Labels are: 1-first(lower) amp, 2-second(higher) amp
else
    paramsReadIn = xlsread('StimParameters_for2AFC.xlsx', 3, 'B3:I7');
% paramsReadIn = xlsread('StimParameters_for2AFC.xlsx', 3, 'B3:I6');
    labels = repelem([1]',5);
end
...
for i=1:numTrials
    ...
    diffAmps(i) = paramsTest(i,6) - paramsTest(i,1);
    ...
    disp([displayTrial, sprintf(', Charge = %.1f vs. %.1f A*us', trainA_chargeDelivered(i), trainB_chargeDelivered(i))])
...
% percentChangeCharge = round(percentChangeCharge);
% 
% if strcmp(whichQuestion, 'Intensity')
%     percentChangeCharge_Labeled = cell(1,3);
%     results_Labeled = cell(1,3);
%     uniqueChangeAmps = cell(1,3);
%     pCorrect = cell(1,3);
%     sd = cell(1,3);
%     
%     for i=1:2 % loop through the three different parameters that were changed
%         percentChangeCharge_Labeled{i} = percentChangeCharge(labelsTest==i);
%         results_Labeled{i} = results(labelsTest==i);
%         uniqueChangeAmps{i} = unique(percentChangeCharge_Labeled{i});
%         nCorrect = zeros(1,length(uniqueChangeAmps{i}));
%         nTrials = zeros(1,length(uniqueChangeAmps{i}));
%         
%         for ii=1:length(uniqueChangeAmps{i})
%             id = percentChangeCharge_Labeled{i} == uniqueChangeAmps{i}(ii);
%             nTrials(ii) = sum(id);
...
%     end
%     
%     min_percentChange = min(percentChangeCharge);
%     max_percentChange = max(percentChangeCharge);
%     
%     figure
%     for i=1:2
%         subplot(1,2,i)
%         errorbar((uniqueChangeAmps{i}),100*pCorrect{i},100*sd{i},'bo','MarkerFaceColor','b');
%         ylim([0 100]);
...
%% Plot results if we ran a intensity question--EDITED!

if strcmp(whichQuestion, 'Intensity')
    diffAmps_Labeled = cell(1,length(unique(labels)));
    results_Labeled = cell(1,length(unique(labels)));
    uniqueChangeAmps = cell(1,length(unique(labels)));
    pCorrect = cell(1,length(unique(labels)));
    sd = cell(1,length(unique(labels)));
    
    for i=1:length(unique(labels)) % loop through the base amplitudes that were used
        diffAmps_Labeled{i} = abs(diffAmps(labelsTest==i));
        results_Labeled{i} = results(labelsTest==i);
        uniqueChangeAmps{i} = unique(diffAmps_Labeled{i});
        nCorrect = zeros(1,length(uniqueChangeAmps{i}));
        nTrials = zeros(1,length(uniqueChangeAmps{i}));
        
        for ii=1:length(uniqueChangeAmps{i})
            id = diffAmps_Labeled{i} == uniqueChangeAmps{i}(ii);
            nTrials(ii) = sum(id);
            nCorrect(ii) = sum(results_Labeled{i}(id)==1);
        end
        pCorrect{i} = nCorrect./nTrials;
    ...
    min_ampDelta = min(abs(diffAmps));
    max_ampDelta = max(abs(diffAmps));
    
    % plot together
    figure
    for i=1:length(unique(labels))
        %         subplot(1,2,i)
        errorbar((uniqueChangeAmps{i}),100*pCorrect{i},100*sd{i}, 'o')%,'bo','MarkerFaceColor','b');
        hold on
        ylim([0 100]);
        xlim([min_ampDelta-5, max_ampDelta+5]);
    end
    %     subplot(1,2,1)
    %     ylabel('Proportion correct')
    %     title(['Amp 1 = ', num2str(min(paramsTest(find(labelsTest==1,1),1), paramsTest(find(labelsTest==1,1),6))), 'uA'])
    %     subplot(1,2,2)
    %     title(['Amp 2 = ', num2str(min(paramsTest(find(labelsTest==2,1),1), paramsTest(find(labelsTest==2,1),6))), 'uA'])
    %     xlabel('Amp delta')
    
    ylabel('Proportion correct')
    legend({['Amp 1 = ', num2str(min(paramsTest(find(labelsTest==1,1),1), paramsTest(find(labelsTest==1,1),6))), 'uA'], ...
        ['Amp 2 = ', num2str(min(paramsTest(find(labelsTest==2,1),1), paramsTest(find(labelsTest==2,1),6))), 'uA']})
    xlabel('Amp delta')
...
% Psychometric function variables (guesses to start the fit):
p.g = 0.5;
p.e = (.5)^(1/3); % This is the threshold level that you want to plot and find
% note that p.e can be set to anything within reason and you will get the
% same curve, it will just return different values for p.b and p.t becuase
% the slope and threshold values change along the curve
pThresh = p.e*100; % this var is used in the plotPsycho function called below...
p.t = 100; % guess for threshold yeilding 79% performance
p.b = 5; % slope
...
% plot together
colors = [[153, 194, 255]/255;
    [0, 102, 255]/255
    [0, 71, 179]/255];

JND = zeros(length(unique(labels)),1);
baseAmps = [min(paramsReadIn(:,1)), max(paramsReadIn(:,1))];
figure
for i=1:length(unique(labels))
    resultsStruct.intensity = diffAmps_Labeled{i};
    resultsStruct.response = results_Labeled{i};
    
    err = fitPsychometricFunction(p,resultsStruct,'Weibull');
    [pBest, err2] = fit('fitPsychometricFunction',p,{'b','t'},resultsStruct,'Weibull');
    JND(i) = pBest.t;
    
    % Plot the parametric psychometric function:
    if ~exist('x_psych', 'var')
        xlim = get(gca,'XLim');
        if xlim(2) > max(resultsStruct.intensity)
            x_psych = linspace(0,max(xlim),101);
        else
            x_psych = linspace(0,max(resultsStruct.intensity),101);
        end
    end
    subplot(1,2,i)
    plotPsycho(resultsStruct,['Change in current amplitude (\muA) for ',num2str(baseAmps(i)), ' uA'],pBest,'Weibull');
end
...
%         uniqueChangeAmps{i} = unique(percentChangeCharge_Labeled{i});
%         nSame = zeros(1,length(uniqueChangeAmps{i}));
%         nTrials = zeros(1,length(uniqueChangeAmps{i}));
%         
%         for ii=1:length(uniqueChangeAmps{i})
%             id = percentChangeCharge_Labeled{i} == uniqueChangeAmps{i}(ii);
...
%         subplot(1,3,i)
%         bar(pSame{i})
%         set(gca,'XTick',1:length(pSame{i}),'XTickLabel',num2str(uniqueChangeAmps{i}))
%     end
%     subplot(1,3,1)
